...
`calculate_eaa_esgsi` is the executable identifier. `E-AA-ESGSI` remains the display label.
...
| Metrics | `EASS`, `IR`, `UPR`, `ESGSI`, `EAA_ESGSI`, `IMBALANCE` | Raw and risk-direction values are separate |
...
| E-AA-ESGSI | `ESGSI + action + indeterminate + planning penalties` | Penalty weights pending |
