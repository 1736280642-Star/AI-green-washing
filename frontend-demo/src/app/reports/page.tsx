...
export default function ReportsPage() {
  ...
  const yearOptions = useMemo(() => Array.from(new Set(companyRecords.filter((record) => record.companyId === company).map((record) => record.reportYear))).sort((a, b) => b - a), [company, companyRecords]);
    ...
    async function poll() {
      try {
        ...
        if (job.status === "completed") {
          const record = await analysisRepository.getCompany(job.resultCompanyId ?? company, "success", Number(year));
  ...
  }, [company, jobId, file?.name, notify, showToast, year]);
  ...
  async function start(candidate = file) { if (!candidate) return; try { const job = await analysisRepository.createAnalysisJob({ companyId: company, reportYear: Number(year), fileName: candidate.name, fileSize: candidate.size }); setCompleted([]); setTaskError(undefined); setResultRecord(null); setPhase("validating"); setJobId(job.jobId); } catch (reason) { showToast(reason instanceof Error ? reason.message : "无法创建检测任务"); } }
