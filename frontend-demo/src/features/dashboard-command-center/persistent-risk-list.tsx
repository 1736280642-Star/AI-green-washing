...
export function PersistentRiskList({ items, selectedCompanyId, onSelect, onCompare, expanded = false, onExpand }: { items: DashboardWatchItem[]; selectedCompanyId: string | null; onSelect: (id: string) => void; onCompare: (id: string) => void; expanded?: boolean; onExpand?: () => void }) {
  return <section className={`cc-panel cc-watch-panel ${expanded ? "cc-panel-expanded" : ""}`}><CommandPanelHeading eyebrow="PRIORITY" title="持续高风险公司" detail={expanded ? "连续三年高风险带 · 建议优先复核" : undefined} onExpand={expanded ? undefined : onExpand} expandLabel="展开持续高风险公司"/><div className="cc-watch-list">{items.length ? items.map((item, index) => <div key={item.companyId} className={`cc-watch-row ${selectedCompanyId === item.companyId ? "selected" : ""}`}>
