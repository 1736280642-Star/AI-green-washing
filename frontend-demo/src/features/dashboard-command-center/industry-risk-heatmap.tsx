...
const heatmapColors = ["#3e7083", "#39a477", "#7fc85f", "#f0bd4f", "#ff765f"];

export function IndustryRiskHeatmap({ data, selectedFactor, onSelectIndustry, expanded = false, onExpand }: { data: DashboardIndustryRiskCell[]; selectedFactor: DashboardTriadCode | null; onSelectIndustry: (industry: string) => void; expanded?: boolean; onExpand?: () => void }) {
  ...
  const option = useMemo<EChartsOption>(() => ({
    ...
    tooltip: {
      position: "top", backgroundColor: "rgba(4,18,9,.96)", borderColor: "rgba(59,220,131,.35)", textStyle: { color: "#e5f3e9", fontSize: 12 },
    ...
    xAxis: { type: "category", data: metricCodes.map((code) => labels[code]), axisLabel: { color: "#9bb2a2", fontSize: expanded ? 14 : 13 }, axisLine: { lineStyle: { color: "rgba(79,168,107,.22)" } }, axisTick: { show: false } },
    yAxis: { type: "category", data: industries, axisLabel: { color: "#9bb2a2", fontSize: expanded ? 14 : 13, width: expanded ? 86 : 68, overflow: "truncate" }, axisLine: { show: false }, axisTick: { show: false } },
    visualMap: { show: false, min: 0, max: 1, inRange: { color: heatmapColors } },
    series: [{ type: "heatmap", data: data.map((item) => ({ value: [metricCodes.indexOf(item.metricCode), industries.indexOf(item.industry), item.medianRiskValue], source: item, itemStyle: { borderColor: "rgba(2,10,5,.5)", borderWidth: 1.5, opacity: selectedFactor == null || (selectedFactor === "RHETORIC_CONTENT" && item.metricCode === "ESGSI") || (selectedFactor === "ACTION_SUBSTANCE" && item.metricCode === "EASS") || (selectedFactor === "AMBIGUITY_VERIFICATION" && (item.metricCode === "IR" || item.metricCode === "UPR")) ? 1 : .34 } })), emphasis: { itemStyle: { borderColor: "#eff8f2", borderWidth: 2, shadowBlur: 12, shadowColor: "rgba(121,227,153,.28)" } } }],
